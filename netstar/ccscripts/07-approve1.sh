...
VERBOSE=false
TMPDIR=./tmp
CC_END_POLICY="OR('${OP_NAME^}MSP.peer','${ORG1_NAME^}MSP.peer','${ORG2_NAME^}MSP.peer')"
...
sed -e "s/__Org1__/${ORG1_NAME^}/g" \
  	...
  	-e "s/__OP__/${OP_NAME^}/g" \
      ${PWD}/configtx/Org1Org2/collections_config3.json > ${CC_COLL_CONFIG}
